...
if __name__ == "__main__":
    ...
    while ordenado == False: #Recorre todo hasta que no esté desordenado y por tanto no entre al if
        ...
        for i in range(len(SecuenciaDesordenada)-1): 
            if SecuenciaDesordenada[i] > SecuenciaDesordenada[i+1]: #compara cada elemento con su elemento de su izquierda
                guardarNumero = SecuenciaDesordenada[i] #guarda el número de la derecha para sustituir y hacer el cambio por el de su izquierda que es menor
